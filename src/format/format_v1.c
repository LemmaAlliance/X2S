...
{
    size_t category_len =
        (obj->metadata && obj->metadata->category) ? strlen(obj->metadata->category) : 0;
    size_t extension_len =
        (obj->metadata && obj->metadata->extension) ? strlen(obj->metadata->extension) : 0;
    size_t filename_len =
        (obj->metadata && obj->metadata->filename) ? strlen(obj->metadata->filename) : 0;

    if (fwrite(&obj->size, sizeof(size_t), 1, f) != 1)
        return 0;
    if (fwrite(&category_len, sizeof(size_t), 1, f) != 1)
        return 0;
    if (fwrite(&extension_len, sizeof(size_t), 1, f) != 1)
        return 0;
    if (fwrite(&filename_len, sizeof(size_t), 1, f) != 1)
        return 0;
    ...
    if (obj->acl && acl_count > 0) {
        for (size_t i = 0; i < acl_count; i++) {
            ...
            if (fwrite(e->user_id, 1, 16, f) != 16)
                return 0;
            if (fwrite(&e->permissions, sizeof(uint32_t), 1, f) != 1)
                return 0;
        }
    }

    if (fwrite(obj->data_hash, 1, 32, f) != 32)
        return 0;

    if (category_len > 0 && obj->metadata && obj->metadata->category)
        if (fwrite(obj->metadata->category, 1, category_len, f) != category_len)
            return 0;
    if (extension_len > 0 && obj->metadata && obj->metadata->extension)
        if (fwrite(obj->metadata->extension, 1, extension_len, f) != extension_len)
            return 0;
    if (filename_len > 0 && obj->metadata && obj->metadata->filename)
        if (fwrite(obj->metadata->filename, 1, filename_len, f) != filename_len)
            return 0;

    size_t metadata_count = (obj->metadata) ? obj->metadata->metadata_count : 0;
    if (fwrite(&metadata_count, sizeof(size_t), 1, f) != 1)
        return 0;
    for (size_t i = 0; i < metadata_count; i++) {
        ...
        if (fwrite(&key_len, sizeof(size_t), 1, f) != 1)
            return 0;
        if (fwrite(k, 1, key_len, f) != key_len)
            return 0;
        if (fwrite(&value_len, sizeof(size_t), 1, f) != 1)
            return 0;
        if (fwrite(v, 1, value_len, f) != value_len)
            return 0;
    }
...
{
    if (fwrite(&capacity, sizeof(size_t), 1, f) != 1)
        return 0;
    if (fwrite(&count, sizeof(size_t), 1, f) != 1)
        return 0;
    if (count > 0 && ids)
        if (fwrite(ids, 32, count, f) != count)
            return 0;
    return 1;
}
...
static int read_data_hash_body(FILE* f, unsigned char hash[32])
{
    ...
    if (fread(&data_len, sizeof(size_t), 1, f) != 1)
        return 0;
    if (fread(&cat_len, sizeof(size_t), 1, f) != 1)
        return 0;
    if (fread(&ext_len, sizeof(size_t), 1, f) != 1)
        return 0;
    if (fread(&file_len, sizeof(size_t), 1, f) != 1)
        return 0;
    ...
    if (fseek(f, (off_t)acl_count * (16 + sizeof(uint32_t)), SEEK_CUR) != 0)
        return 0;
...
const FormatVtable format_registry[] = {
    {0, "legacy", read_metadata_body, NULL, read_index_body, NULL, read_users_body, NULL,
     read_data_hash_body},
