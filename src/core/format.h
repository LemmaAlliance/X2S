...
#define X2S_FILE_TYPE_REFRESH 0x05
...
{
    X2sFileHeader hdr = {{X2S_MAGIC_0, X2S_MAGIC_1}, file_type, version};
    return fwrite(&hdr, sizeof(hdr), 1, f) == 1;
}

static inline int read_header(FILE* f, X2sFileHeader* out)
{
    return fread(out, sizeof(*out), 1, f) == 1;
}

static inline int header_matches(const X2sFileHeader* hdr, uint8_t expected_type)
{
    return hdr->magic[0] == X2S_MAGIC_0 && hdr->magic[1] == X2S_MAGIC_1 &&
